...
count  total (s)   self (s)
    1              0.000025   let l:ag_executable = get(split(g:agprg, " "), 0)
    ...
    1              0.000091     let l:grepargs = a:args . join(a:000, ' ')
    1              0.000004   end
    ...
    1   0.113034   0.065676     silent execute a:cmd . " " . escape(l:grepargs, '|')
    1              0.000004   finally
    1              0.000015     let &grepprg=grepprg_bak
    1              0.000008     let &grepformat=grepformat_bak
    1              0.000004   endtry
                            
    1              0.000012   if a:cmd =~# '^l'
                                let l:match_count = len(getloclist(winnr()))
                              else
    1              0.000370     let l:match_count = len(getqflist())
    1              0.000004   endif
                            
    1              0.000037   if a:cmd =~# '^l' && l:match_count
                                exe g:ag_lhandler
                                let l:apply_mappings = g:ag_apply_lmappings
                                let l:matches_window_prefix = 'l' " we're using the location list
                              elseif l:match_count
    1   0.016833   0.001322     exe g:ag_qhandler
    ...
    1              0.000035       nnoremap <silent> <buffer> h  <C-W><CR><C-w>K
    ...
    1              0.000037       exe 'nnoremap <silent> <buffer> e <CR><C-w><C-w>:' . l:matches_window_prefix .'close<CR>'
    1              0.000031       exe 'nnoremap <silent> <buffer> go <CR>:' . l:matches_window_prefix . 'open<CR>'
    1              0.000028       exe 'nnoremap <silent> <buffer> q  :' . l:matches_window_prefix . 'close<CR>'
                            
    1              0.000095       exe 'nnoremap <silent> <buffer> gv :let b:height=winheight(0)<CR><C-w><CR><C-w>H:' . l:matches_window_prefix . 'open<CR><C-w>J:exe printf(":normal %d\<lt>c-w>_", b:height)<CR>'
                                  " Interpretation:
                                  " :let b:height=winheight(0)<CR>                      Get the height of the quickfix/location list window
                                  " <CR><C-w>                                           Open the current item in a new split
                                  " <C-w>H                                              Slam the newly opened window against the left edge
