...
count  total (s)   self (s)
    ...
    1              0.000009 if !exists("g:ag_apply_qmappings")
    1              0.000008   let g:ag_apply_qmappings=1
    1              0.000004 endif
    ...
    1              0.000008 if !exists("g:ag_qhandler")
    ...
    1              0.000015 function! ag#GetDocLocations()
                              let dp = ''
...
count  total (s)   self (s)
    1              0.000024   let l:ag_executable = get(split(g:agprg, " "), 0)
                            
                              " Ensure that `ag` is installed
    1              0.000039   if !executable(l:ag_executable)
                                echoe "Ag command '" . l:ag_executable . "' was not found. Is the silver searcher installed and on your $PATH?"
                                return
                              endif
    ...
    1              0.000093     let l:grepargs = a:args . join(a:000, ' ')
    1              0.000004   end
    ...
    1              0.000007   let grepformat_bak=&grepformat
    1              0.000005   try
    1              0.000015     let &grepprg=g:agprg
    1              0.000007     let &grepformat=g:agformat
    1   0.308564   0.104072     silent execute a:cmd . " " . escape(l:grepargs, '|')
    1              0.000018   finally
    1              0.000036     let &grepprg=grepprg_bak
    1              0.000015     let &grepformat=grepformat_bak
    1              0.000008   endtry
                            
    1              0.000035   if a:cmd =~# '^l'
                                let l:match_count = len(getloclist(winnr()))
                              else
    1              0.000050     let l:match_count = len(getqflist())
    1              0.000006   endif
                            
    1              0.000019   if a:cmd =~# '^l' && l:match_count
                                exe g:ag_lhandler
                                let l:apply_mappings = g:ag_apply_lmappings
                                let l:matches_window_prefix = 'l' " we're using the location list
                              elseif l:match_count
    1   0.021393   0.001938     exe g:ag_qhandler
    1              0.000009     let l:apply_mappings = g:ag_apply_qmappings
    ...
    1              0.000022   if l:match_count
    1              0.000005     if l:apply_mappings
    1              0.000036       nnoremap <silent> <buffer> h  <C-W><CR><C-w>K
    1              0.000020       nnoremap <silent> <buffer> H  <C-W><CR><C-w>K<C-w>b
    1              0.000013       nnoremap <silent> <buffer> o  <CR>
    1              0.000015       nnoremap <silent> <buffer> t  <C-w><CR><C-w>T
    1              0.000021       nnoremap <silent> <buffer> T  <C-w><CR><C-w>TgT<C-W><C-W>
    1              0.000022       nnoremap <silent> <buffer> v  <C-w><CR><C-w>H<C-W>b<C-W>J<C-W>t
                            
    1              0.000040       exe 'nnoremap <silent> <buffer> e <CR><C-w><C-w>:' . l:matches_window_prefix .'close<CR>'
    1              0.000029       exe 'nnoremap <silent> <buffer> go <CR>:' . l:matches_window_prefix . 'open<CR>'
    1              0.000028       exe 'nnoremap <silent> <buffer> q  :' . l:matches_window_prefix . 'close<CR>'
                            
    1              0.000091       exe 'nnoremap <silent> <buffer> gv :let b:height=winheight(0)<CR><C-w><CR><C-w>H:' . l:matches_window_prefix . 'open<CR><C-w>J:exe printf(":normal %d\<lt>c-w>_", b:height)<CR>'
                                  " Interpretation:
                                  " :let b:height=winheight(0)<CR>                      Get the height of the quickfix/location list window
                                  " <CR><C-w>                                           Open the current item in a new split
                                  " <C-w>H                                              Slam the newly opened window against the left edge
    ...
    1              0.000079         echom "ag.vim keys: q=quit <cr>/e/t/h/v=enter/edit/tab/split/vsplit go/T/H/gv=preview versions of same"
    1              0.000003       endif
    1              0.000003     endif
    1              0.000003   else
                                echom 'No matches for "'.a:args.'"'
